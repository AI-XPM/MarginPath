...
device = "cuda"
local_model_path = os.path.abspath("/root/autodl-tmp/qwen/Qwen2-VL-2B-Instruct")


class MPMDataSet(Dataset):  # for toy demo
...
def train():
    ...
    model = Qwen2VLForConditionalGeneration.from_pretrained(
        local_model_path, torch_dtype=torch.bfloat16, attn_implementation="flash_attention_2", device_map="auto",
    ...
    processor = AutoProcessor.from_pretrained(local_model_path, min_pixels=256 * 28 * 28, max_pixels=512 * 28 * 28,
                                              padding_side="right", local_files_only=True)

    train_loader = DataLoader(
        MPMDataSet("train_data/MPM.json"),
        batch_size=1,
